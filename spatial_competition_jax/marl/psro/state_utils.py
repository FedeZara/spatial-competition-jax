...
agent 1, we permute the global-state vector so the policy sees itself
in the agent-0 slot.

The global-state layout (from :pyclass:`TrainingWrapper`) is::

    [ blob_channels (permutation-invariant) | agent_0_feats | agent_1_feats | … ]

The blob channels (presence map, avg-price map, …) aggregate across all
sellers and are therefore already symmetric.  Only the per-agent scalar
features at the tail need swapping.
"""
